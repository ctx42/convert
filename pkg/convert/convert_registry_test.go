...
func Test_Registry_register(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		// --- Then ---
		assert.Nil(t, have)
		assert.Same(t, wrp, reg.m[src][dst])
	})
	...
	t.Run("register already registered pair", func(t *testing.T) {
		...
		wrp1 := wrap(cnv)
		reg := &Registry{
			m: map[reflect.Type]map[reflect.Type]*wrapper{src: {dst: wrp0}},
		}
...
func Test_Registry_lookup(t *testing.T) {
	...
	t.Run("existing converter", func(t *testing.T) {
		...
		wrp := wrap(cnv)
		reg := &Registry{
			m: map[reflect.Type]map[reflect.Type]*wrapper{src: {dst: wrp}},
		}
	...
	t.Run("not existing converter - from", func(t *testing.T) {
		...
		// --- When ---
		have := reg.lookup(reflect.TypeFor[int](), dst)
	...
	t.Run("not existing converter - to", func(t *testing.T) {
		...
		src := reflect.TypeFor[uint]()
		dst := reflect.TypeFor[uint8]()
		cnv := func(src uint) (uint8, error) { return uint8(src), nil }
		wrp := wrap(cnv)
		reg := &Registry{
			m: map[reflect.Type]map[reflect.Type]*wrapper{src: {dst: wrp}},
		}
		...
		// --- When ---
		have := reg.lookup(src, reflect.TypeFor[int]())
...
func Test_RegisterConverter(t *testing.T) {
	...
	t.Run("it should never happen", func(t *testing.T) {
		// --- Given ---
		wrongTypes := func(src float32) (int, error) { return 0, nil }
		cnv := func(from uint) (uint8, error) { return uint8(from), nil }
		reg := &Registry{
